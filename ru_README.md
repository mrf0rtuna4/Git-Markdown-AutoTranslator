# GitHub Markdown Файлы Автопереводчик
<div align="center">
...
Чтобы использовать это действие, создайте в своем репозитории файл рабочего процесса (например, `.github/workflows/translate.yml`) со следующим содержимым:
...
- `FILES`: список файлов для перевода, разделенных запятыми.
- `LANGS`: список языков, разделенных запятыми, для генерации.
- `DEBUG`: Установите `True`, чтобы включить подробное ведение журнала процесса перевода. Это полезно для устранения неполадок, но может привести к появлению подробного вывода.
- `MAX_LINELENGTH_`: Указывает максимально допустимую длину строки для перевода. **Используйте с осторожностью!** Установка слишком низкого значения может привести к ошибкам или неполному переводу.

> [!ВНИМАНИЕ]
> Избегайте использования `MAX_LINELENGTH_` без полного понимания его последствий.
> Неправильная конфигурация (например, очень низкое значение) может привести к неожиданному поведению или сбоям перевода.


## Пример

Например, если вы хотите создать файлы для сербского, итальянского и английского языков, ваша конфигурация будет выглядеть следующим образом:
...
Данный проект лицензирован по лицензии MIT — подробности см. в файле [LICENSE](ЛИЦЕНЗИЯ).