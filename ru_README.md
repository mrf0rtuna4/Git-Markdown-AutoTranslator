# Автопереводчик файлов GitHub Markdown
<div align="center">
...
- `ФАЙЛЫ`: список файлов для перевода, разделенных запятыми.
- `LANGS`: список языков для генерации, разделенных запятыми.
- `DEBUG`: установите значение `True`, чтобы включить подробное протоколирование процесса перевода. Это полезно для устранения неполадок, но может привести к созданию подробного вывода.
- `MAX_LINELENGTH_`: определяет максимально допустимую длину строки для перевода. **Используйте с осторожностью!** Установка слишком низкого значения может привести к ошибкам или неполному переводу. (По умолчанию: 500)
